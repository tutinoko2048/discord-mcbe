...
const lengthFilterSchema = z.object({
  on_fail: z.enum(['cancel', 'shorten'])
  ...
  max_content_lines: z.number().int().positive().optional()
    .describe('Maximum number of lines in the content'),
}).describe('LengthFilter');

const regexFilterSchema = z.object({
  on_fail: z.enum(['cancel', 'replace'])
    .describe('Action to take when content matches the regex pattern'),
  ignore_pattern: z.string()
    .describe('Regular expression for content to ignore certain content'),
...
const botSchema = z.object({
  ...
  discord_message_filter: z.union([
    messageFilterSchema,
    z.array(messageFilterSchema)
  ]).describe('Filters to apply to messages sent from Discord to Minecraft. Can be a single filter or an array of filters to apply in order.'),
});
