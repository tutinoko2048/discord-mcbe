...
- アップデート方法
  - バージョン取得
    ...
    - リリースアセットから `version.json` を取得して `minimumLauncherVersion` (連番) をチェックし、アップデーター自体が古くないか確認する。
  - アップデート処理
...
- [ ] `version.ts`: GitHub Releases API を使用した実際のリリース取得処理の実装（`launcher@v*` タグの除外、アセットURLのパース）
- [ ] `version.ts`: アセットから `version.json` をフェッチし、`minimumLauncherVersion` を検証するロジックの実装
...
- [ ] `install.ts`: インストール完了時、`app/.VERSION` に現在のバージョンを書き出す処理
- [ ] `run.bat` / `run.sh`: `app` フォルダが存在しない場合に `updater` を自動起動し、その後 `BUN_BE_BUN=1` で本体を起動するスクリプトの整備
- [ ] (仕組み化) GitHub Actionsで `discord-mcbe-*.tar.gz` (本体・環境一式) と `version.json` を生成してリリースに上げるCI/CDの整備
