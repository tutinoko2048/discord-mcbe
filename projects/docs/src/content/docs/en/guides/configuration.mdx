...
<Tabs>
  ...
  <TabItem label="BDS">
    ...
    by the add-on, and `DEFAULT_WORLD_NAME` changes its initial world name. Restart BDS after changing these
    values.
    ...
    ```json
    {
      ...
      "BRIDGE_PORT": 23191,
      "DEFAULT_WORLD_NAME": "discord-mcbe-bds"
    }
    ```

    A world name saved with `/dmc:setid` takes precedence over `DEFAULT_WORLD_NAME`.
