...
<Tabs>
  ...
  <TabItem label="BDS">
    ...
    `config/195cf53c-ae53-4437-82d9-a84e08694ffc/variables.json` file. `BRIDGE_HOST` changes the host seen by
    the add-on, and `DEFAULT_CLIENT_ID` changes its initial Client ID. Restart BDS after changing these
    values.
  </TabItem>
</Tabs>

`variables.json` belongs in the `config` folder in the BDS installation folder, not in the add-on. Use this content:
