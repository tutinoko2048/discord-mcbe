...
<Aside type="note" title="Running on separate machines">
  In the BDS configuration, set `BRIDGE_URL` to discord-mcbe's WebSocket URL. Use the same port as
  `BRIDGE_PORT` in the bot's `.env`. See the BDS tab in [Change connection
  settings](/en/guides/configuration/#change-connection-settings) for details.
</Aside>
