...
## <Icon name="shield-check" />BDS接続を認証する (任意)

BDSのserver-net接続は、初期状態では認証されません。Bearer認証を使う場合は、`.env`へ任意のトークンを追加し、カスタムスクリプトから認証処理を設定します。
...
BDS側の`variables.json`にも同じトークンを設定します。
...
独自の認証を使う場合は、`setAuthenticator`へリクエストを受け取る関数を渡してください。関数は`boolean`または`Promise<boolean>`を返します。認証に失敗した接続は、ワールドセッションが作られる前に拒否されます。
...
全メソッドと型は[APIリファレンス](/reference/)を参照してください。
