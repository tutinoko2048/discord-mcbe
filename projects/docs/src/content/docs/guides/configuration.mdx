...
<Tabs>
  ...
  <TabItem label="BDS">
    ...
    を作成し、`.env`の`BRIDGE_PORT`と同じ値を`BRIDGE_PORT`へ設定します。`BRIDGE_HOST`でアドオンから見たサーバーのホスト名、`DEFAULT_WORLD_NAME`で初期Client
    ...
    `/dmc:setid`で保存済みのworld nameがある場合は、`DEFAULT_WORLD_NAME`より優先されます。
