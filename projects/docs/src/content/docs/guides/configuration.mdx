...
| `bot.show_death_messages`      | プレイヤーの死亡メッセージをDiscordへ送る。既定値は`true`     |
...
<Tabs>
  ...
  <TabItem label="BDS">
    ...
    を作成し、`.env`の`BRIDGE_PORT`と同じ値を`BRIDGE_PORT`へ設定します。`BRIDGE_HOST`でアドオンから見たサーバーのホスト名、`DEFAULT_CLIENT_ID`で初期Client
