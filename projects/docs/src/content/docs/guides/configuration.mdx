---
title: 設定
description: .env、config.json、メッセージフィルター、表示文言を設定します。
---
...
- `.env`: トークン、DiscordのID、待受ポートなど、環境ごとに異なる機密値
...
| `BRIDGE_PORT`   |      | `23191` | BDSアドオンが接続するScriptBridgeポート               |
...
| `translationOverrides`         | 組み込み翻訳の一部を上書きするマップ                          |
| `debug`                        | 詳細ログを有効にする                                          |

未知のキーは警告とともに無視されます。型や値が不正な場合はエラー箇所を表示して起動を中止します。
...
<Tabs>
  ...
  <TabItem label="BDS">
    `.env`の`BRIDGE_PORT`と、独自にビルドしたBDSアドオンの`BridgeClient({port}
    )`を同じ値にします。配布済みアドオンは`23191`固定です。
  </TabItem>
</Tabs>
